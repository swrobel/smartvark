class WelcomeController < ApplicationController
  ...
  def deals
    ...
    @offers = Offer.select('DISTINCT offers.*').includes([:businesses,:user,:offer_type,:category]).joins(:businesses).where({:businesses => [:id + Business.ids_close_to(loc,250)]}).active.order(:created_at.desc)
  ...
  def mydeals
    ...
    loc = geo_location
    @offers = Offer.select('DISTINCT offers.*').includes([:businesses,:user,:offer_type,:category]).joins(:businesses).where({:businesses => [:id + Business.ids_close_to(loc,250)]} & (:category_id + Category.subtree_of(@category_id))).active.order(:created_at.desc)
